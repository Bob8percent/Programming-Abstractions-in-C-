...
{
	int count = 1;
	bool result= solveMaze(startSquare, count);
	//std::cout << count << std::endl;
	return result;
}

bool Maze::solveMaze(IPoint pos, int& count)
{
...
bool Maze::findSolutionPath(Vector<IPoint>& path)
{
	return findSolutionPath(startSquare, path);
}

bool Maze::findSolutionPath(IPoint pos, Vector<IPoint>& path)
{
	if (isOutside(pos))return true;
	if (wallExists(pos) || isMarked(pos)) return false;
	...
	{
		IPoint nextPos = adjacentIPoint(pos, dir);
		if (findSolutionPath(nextPos, path))
		{
			return true;
		}
	}
