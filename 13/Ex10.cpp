	...
	while (1) {
		...
		if (state == s0) {
			...
			case 'R':
				for (std::size_t i = 1; i < line.length(); ++i) {
					if (line.at(i) == '/') {
						if (!eb.search(word))break;
						for (std::size_t j = 0; j < word.length(); ++j)eb.moveCursorBackward();
						for (std::size_t j = 0; j < word.length(); ++j)eb.deleteCharacter();
						for (std::size_t j = i + 1; j < line.length(); ++j)eb.insertCharacter(line.at(j));
					}
					word += line.at(i);
				}
				return;
