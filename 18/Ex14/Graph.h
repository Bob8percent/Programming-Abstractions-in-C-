...
public:
	...
	Vector<ArcType*> findShortestPath(const std::string& startName, const std::string& endName) const;
	Vector<ArcType*> findShortestPath(NodeType* start, NodeType* end) const;
	...
	Graph<NodeType, ArcType> findMinimumSpanningTree() const;

	std::set<NodeType*> findDominatingSet() const;
...
template<typename NodeType, typename ArcType>
bool Graph<NodeType, ArcType>::hasPathUsingBFS(const std::string& s1, const std::string& s2) const {
	return hasPathUsingBFS(nodeMap.at(s1), nodeMap.at(s2));	//	nodeMap[~]という書き方はconst関数内では許されない。at()に関してはconst参照返しの関数も用意されている
}

template<typename NodeType, typename ArcType>
bool Graph<NodeType, ArcType>::hasPathUsingBFS(NodeType* n1, NodeType* n2) const {
...
template<typename NodeType, typename ArcType>
Vector<ArcType*> Graph<NodeType, ArcType>::findShortestPath(const std::string& startName, const std::string& endName) const{
...
template<typename NodeType, typename ArcType>
Vector<ArcType*> Graph<NodeType, ArcType>::findShortestPath(NodeType* start, NodeType* end) const{
	Vector<ArcType*> path;
...
//	Kruskalアルゴリズムにより最小スパニングツリーを見つける
//	gとthisの区別に気をつけたい
template<typename NodeType, typename ArcType>
Graph<NodeType, ArcType> Graph<NodeType, ArcType>::findMinimumSpanningTree() const {
	PQueue<ArcType*> q;
	for (ArcType* a : arcs) {
		q.enqueue(a, a->cost);
	}
	...
	while (!q.isEmpty()) {
		ArcType* cp = q.dequeue();
		std::string s1 = cp->start->name;
		std::string s2 = cp->end->name;
		if (!g.hasPathUsingBFS(s1, s2)) {
			//std::cout << s1 << "  " << s2 << std::endl;
			g.addArc(s1, s2, cp->cost);
			g.addArc(s2, s1, cp->cost);
		}
	}
...
std::set<NodeType*> Graph<NodeType, ArcType>::findDominatingSet() const {
	...
	PQueue<NodeType*> q;
	for (NodeType* n : nodes) {
		int value = -1 * n->arcs.size();
		q.enqueue(n, value);	//	次数が大きいノードから(降順に)処理したい
	}
	...
	while (1) {
		...
		for (NodeType* sn : s) {
			for (ArcType* sa : sn->arcs) {
				NodeType* san = sa->end;
				if (san == cp)isInsert = false;
			}

			if (!isInsert)break;
		}
		if (isInsert) s.insert(cp);

		//	sがグラフを支配していたら終了
		std::set<NodeType*> snodes;
		for (NodeType* sn : s) {
			snodes.insert(sn);
			for (ArcType* a : sn->arcs) {
				snodes.insert(a->end);
			}
		}

		//std::cout << "snodes : "<<snodes.size() << std::endl;
		//std::cout << "nodes  : " << nodes.size() << std::endl << std::endl;
		if (nodes.size() == snodes.size())return s;
	}
}
