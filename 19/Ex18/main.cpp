...
void runBASIC() {
	...
	for (int i = 0; i < lines.size(); ++i) {
		...
		if (cmd == "LET") {
			...
			ValueType value;
			std::cin >> value;
			context.setValue(scanner.nextToken(), value);
		}
		else if (cmd == "PRINT") {
			exp = parseExp(scanner);
			...
			std::string e1 = scanner.nextToken();
			std::string op = scanner.nextToken();
			std::string e2 = scanner.nextToken();
			//	e1,e2を定数か変数のいずれかであると想定して値に変換
			ValueType v1 = context.isDefined(e1) ? context.getValue(e1) : stringToDouble(e1);
			ValueType v2 = context.isDefined(e2) ? context.getValue(e2) : stringToDouble(e2);
			...
			if (scanner.nextToken() == "THEN") {
				...
				std::cerr << "ERROR : void runBASIC() : "
					<< "IFを指定したら3つのtokenを指定した後、THENの指定が必要です" << std::endl;
				std::exit(EXIT_FAILURE);
			}
		}
	}
