...
public:
	...
	////	引数のラムダ式を順に実行する
	//void funcForEach(std::function<void(KeyType)> lambda);

	//	イテレーター
	typedef BSTIterator<KeyType, ValueType> iterator;
	iterator begin() {
...
private:
	...
	//void funcForEach(std::function<void(KeyType)> lambda, BSTNode* node);

	BSTNode* root;
	int count;

	friend class BSTIterator<KeyType, ValueType>;
};
...
BinarySearchTree<KeyType, ValueType>::remove(BSTNode*& node, int& direction, const KeyType& key) {
	if (!node) {
		std::cerr << "ERROR : BSTNode* BinarySearchTree::remove(BSTNode * &node, int& direction, const KeyType & key) : "
			<< "要素が見つかりません" << std::endl;
		std::exit(EXIT_FAILURE);
		return nullptr;
	}
...
//template<typename KeyType, typename ValueType>
//void BinarySearchTree<KeyType, ValueType>::funcForEach(std::function<void(KeyType)> lambda) {
//	funcForEach(lambda, root);
//}
//
//template<typename KeyType, typename ValueType>
//void BinarySearchTree<KeyType, ValueType>::funcForEach(std::function<void(KeyType)> lambda, BSTNode* node) {
//	if (!node)return;
//
//	funcForEach(lambda, node->left);
//	lambda(node->key);
//	funcForEach(lambda, node->left);
//}
...
public:
	...
	ValueType& operator*() {
		if (q.empty()) {
			std::cerr << "ERROR : ValueType& BSTIterator::operator*() : "
				<< "qが空です" << std::endl;
			std::exit(EXIT_FAILURE);
		}
	...
	bool operator==(const BSTIterator < KeyType, ValueType > & rhs) {
		if (q.empty() && rhs.q.empty())return true;
		if (q.empty() || rhs.q.empty())return false;
	...
	BSTIterator<KeyType, ValueType>& operator++() {
		if (q.empty()) {
			std::cerr << "ERROR : ValueType& BSTIterator::operator++() : "
				<< "qが空です" << std::endl;
			std::exit(EXIT_FAILURE);
	...
	BSTIterator<KeyType, ValueType> operator++(int) {
		if (q.empty()) {
			std::cerr << "ERROR : ValueType& BSTIterator::operator++(int) : "
				<< "qが空です" << std::endl;
			std::exit(EXIT_FAILURE);
		}
		BSTIterator copy(*this);	//	デフォルトのコピーコンストラクタ(浅いコピーでOK)
		q.pop();
		return copy;
	}

private:
	typedef BinarySearchTree<KeyType, ValueType>::BSTNode* NodePtr;
	std::queue<NodePtr> q;

	BSTIterator(BinarySearchTree<KeyType, ValueType>::BSTNode* root, int popSize) {
		setQueue(root);
		while (popSize--)q.pop();
	}

	void setQueue(BinarySearchTree<KeyType, ValueType>::BSTNode* node) {
		if (!node)return;
